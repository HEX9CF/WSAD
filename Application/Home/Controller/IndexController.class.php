...
class IndexController extends Controller {
    ...
    public function index(){
		...
		$tops = $pagemodel->where( array('top'=>'true') )->order('pid desc')->select();
		$newmsgs = $msgmodel->order('mid desc')->limit('5')->select();
		$where['img'] = array(
			'NEQ',''
		);
		...
		//获取排行榜数据
		$read_hots = $pagemodel->order(array('read'=>'desc'))->limit('5')->select();
		$zan_hots = $pagemodel->order('zan desc')->limit('5')->select();
		$msg_hots = $pagemodel->order('msg desc')->limit('5')->select();

		//输出
		$this->assign('read_hots',$read_hots);
		$this->assign('zan_hots',$zan_hots);
		$this->assign('msg_hots',$msg_hots);
		$this->assign('newmsgs',$newmsgs);
	...
	public function zan(){
		...
		$pagemodel->where( array('pid'=>$pid) )->save( $insert );
		$url =  "/Home/Index/read?pid=" . $blog['pid'];
